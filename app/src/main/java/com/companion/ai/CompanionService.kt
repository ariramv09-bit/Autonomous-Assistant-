...
class CompanionService : AccessibilityService(), TextToSpeech.OnInitListener {
    ...
    // தன்னாட்சி உரையாடல் நினைவகம் (Conversational Memory History)
    ...
    private fun triggerAutonomousPerception() {
        if (isBusy) {
            Toast.makeText(this, "AI பணியில் உள்ளது...", Toast.LENGTH_SHORT).show()
            return
        }
        ...
        if (apiKey.isEmpty()) {
            speakTamil("முதலில் ஆப்பில் Gemini API Key சேமிக்கவும் Master.")
            return
        }
        ...
        CoroutineScope(Dispatchers.IO).launch {
            try {
                ...
                val systemPrompt = """
                    You are an Autonomous Android Assistant with conversational memory.
                    Inspect the current screen context and ongoing dialogue history to decide the next action.
                    Return ONLY a JSON object in this format without markdown code blocks:
                    {
                      "action": "CLICK" | "SCROLL_DOWN" | "SCROLL_UP" | "GLOBAL_HOME" | "GLOBAL_BACK" | "OPEN_APP" | "SPEAK_ONLY",
                      "x": integer_x,
                      "y": integer_y,
                      "package_name": "package.name if OPEN_APP else empty",
                      "tamil_reply": "Tamil response to speak to Master"
                    }
                """.trimIndent()

                // உரையாடல் நினைவகத்தில் புதிய திரை நிலையைச் சேர்த்தல்
                ...
                val requestContents = JSONArray()
                for (i in 0 until conversationHistory.length()) {
                    requestContents.put(conversationHistory.getJSONObject(i))
                }
                requestContents.put(currentTurn)
                ...
                if (connection.responseCode == HttpURLConnection.HTTP_OK) {
                    ...
                    val pkgName = actionObj.optString("package_name", "")
                    val tamilReply = actionObj.optString("tamil_reply", "திரை ஆய்வு முடிந்தது Master.")

                    // AI பதிலையும் நினைவகத்தில் பதிவு செய்தல்
                    val modelTurn = JSONObject().apply {
                        put("role", "model")
                        put("parts", JSONArray().apply {
                            put(JSONObject().apply { put("text", cleanJson) })
                        })
                    }
                    conversationHistory.put(currentTurn)
                    conversationHistory.put(modelTurn)

                    withContext(Dispatchers.Main) {
                        speakTamil(tamilReply)
                        dispatchAgentAction(action, x, y, pkgName)
                    }
                }
                connection.disconnect()
            } catch (e: Exception) {
                withContext(Dispatchers.Main) {
                    speakTamil("செயல்முறை முடிப்பதில் பிழை ஏற்பட்டது Master.")
                }
            } finally {
                isBusy = false
            }
    ...
    private fun dispatchAgentAction(action: String, x: Int, y: Int, pkgName: String) {
        when (action) {
            "CLICK" -> {
                if (x > 0 && y > 0) {
